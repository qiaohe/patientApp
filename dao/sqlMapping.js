module.exports = {
    ...
    medical: {
        ...
        findRecipesByOrderNo: 'select id, name, specification, quantity, unit, `usage`, dosage from Recipe where orderNo = ?',
        findPrescriptionByOrderNo: 'select name, quantity, unit, discount, receivable from Prescription where orderNo=?',
        ...
        findOrdersBy: 'select m.type, m.orderNo, r.doctorName, r.departmentName, r.hospitalName, r.patientName, m.amount, m.paidAmount, m.paymentType, m.createDate, concat(DATE_FORMAT(r.registerDate, \'%Y-%m-%d \') , s.`name`) as shiftPeriod from MedicalOrder m left JOIN Registration r on r.id=m.registrationId left join ShiftPeriod s on s.id = r.shiftPeriod   where m.orderNo =?'
    }
}
