module.exports = {
    ...
    medical: {
        ...
        findRecipes: 'select r.id as registrationId, h.resumptiveDiagnosis, h.createDate, r.doctorName, r.departmentName, r.hospitalName,r.hospitalId, concat(DATE_FORMAT(r.registerDate, \'%Y-%m-%d \') , s.`name`) as shiftPeriod from MedicalHistory h left JOIN Registration r on h.registrationId = r.id left JOIN ShiftPeriod s on s.id= r.shiftPeriod left JOIN MedicalOrder m ON r.orderNo = m.orderNo where r.patientBasicInfoId=? and m.sendDrugDate is not NULL order by h.createDate desc limit ?, ?',
